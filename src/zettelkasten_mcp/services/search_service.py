...
class SearchService:
    ...
    def find_orphaned_notes(self) -> List[Note]:
        ...
        # Optimised path for SQL backends
        if hasattr(repo, "session_factory"):
            orphans = []
            with repo.session_factory() as session:  # type: ignore[attr-defined]
                notes_with_links = (
                    select(DBNote.id)
                    .outerjoin(
                        DBLink,
                        or_(
                            DBNote.id == DBLink.source_id, DBNote.id == DBLink.target_id
                        ),
                    )
        ...
        # Fallback path for non-SQL backends (e.g., Mongo)
        all_notes = self.zettel_service.get_all_notes()
        outgoing_ids = {note.id for note in all_notes if note.links}
        incoming_ids = set()
        for note in all_notes:
            incoming_ids.update({link.target_id for link in note.links})

        return [
            note
            for note in all_notes
            if note.id not in outgoing_ids and note.id not in incoming_ids
        ]
    ...
    def find_central_notes(self, limit: int = 10) -> List[Tuple[Note, int]]:
        ...
        # Fallback path for non-SQL backends (e.g., Mongo)
        all_notes = self.zettel_service.get_all_notes()
        outgoing_counts = {note.id: len(note.links) for note in all_notes}

        incoming_counts: Dict[str, int] = {}
        for note in all_notes:
            for link in note.links:
                incoming_counts[link.target_id] = (
                    incoming_counts.get(link.target_id, 0) + 1
                )

        note_connections = []
        for note in all_notes:
            total = outgoing_counts.get(note.id, 0) + incoming_counts.get(note.id, 0)
            if total > 0:
